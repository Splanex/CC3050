...
def make_flatfield_corrected_image(darkframe, hdu, flatfield):
    ...
    data_copy = functions.remove_hotpixels(data_copy, hot_pixels, width=2)
...
def make_masterlight():
    with fits.open(sys.argv[1]) as darkframe:
        ...
        with fits.open(sys.argv[3]) as flatfield:
            flatfield_data = np.copy(flatfield["PRIMARY"].data)
            lights = glob.glob(lights_dir+"/*")
            for light in lights:
                with fits.open(light) as hdu:
                    data_copy = np.copy(hdu["PRIMARY"].data)
                    hot_pixels = functions.get_hotpixels(darkframe_data, k=2)

                    corrected_data = functions.remove_hotpixels(data_copy, hot_pixels)
                    #corrected_data = remove_resistant_hotpixels(corrected_data)
        ...
        try:
            hdul = fits.PrimaryHDU(data_copy)
            hdul.writeto(lights_dir+"/masterLight2.fits",overwrite=True)

        except Exception as e:
            print(e)

"""
imgs = glob.glob(imgs_direct+"*.fits")
