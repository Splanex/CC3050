...
def axu_gammaCorrection(data, gamma, max):
    ...
    #data = [(max * ((data[line][collumn] / max) ** (1/gamma))) for line in range(x_res) for collumn in range(y_res)]
    return data


def gammaCorrection(data, gamma=0.4):
    max = data.max()
    k = int(len(data[0])/12)
    inicio = 0
    fim = k
...
def histogramSpread(data, blackPoint=0, whitePoint=65535):
    max = data.max()
    k = int(len(data[0])/12)
    inicio = 0
    fim = k
